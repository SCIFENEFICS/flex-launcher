...
#define HIGHLIGHT_FORMAT "<svg viewBox=\"0 0 %i %i\"><rect x=\"%.2f\" y=\"%.2f\" width=\"%.2f\" height=\"%.2f\" rx=\"%i\" fill=\"#%02X%02X%02X\" fill-opacity=\"%.2f\"%s/></svg>"
...
#define format_highlight(buffer, width, height, corner_radius, fill_color, fill_opacity, outline_buffer) do { float inset=(float)config.highlight_outline_size/2.0f; sprintf_alloc(buffer,HIGHLIGHT_FORMAT,width,height,inset,inset,(float)width-2.0f*inset,(float)height-2.0f*inset,corner_radius,fill_color.r,fill_color.g,fill_color.b,fill_opacity,outline_buffer); } while(0)
